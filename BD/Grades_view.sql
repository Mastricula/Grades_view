...
INSERT INTO Estudiantes (matricula, nombre, apellido, fecha_nacimiento, id_curso, id_usu) VALUES ('2024-0000', 'Miguel', 'Alcantara', '2007-01-12', 6, 8);
...
SELECT CONCAT(e.nombre, ' ', e.apellido) AS Nombres, matricula, curso , materia AS materias, nota_p1 AS P1, nota_p2 AS P2, nota_p3 AS P3, nota_p4 AS P4, SUM(nota_p1 + nota_p2 + nota_p3 + nota_p4) / 4.0 AS Total FROM notas n JOIN estudiantes e ON n.id_estu = e.id_estu JOIN cursos c ON e.id_curso = c.id_curso JOIN materias m ON n.id_materia = m.id_materia WHERE n.id_estu = 1;

SELECT 
    CONCAT(e.nombre, ' ', e.apellido) AS Nombres, 
    e.matricula, 
    c.curso, 
    m.materia AS materias, 
    n.nota_p1 AS P1, 
    n.nota_p2 AS P2, 
    n.nota_p3 AS P3, 
    n.nota_p4 AS P4,
    -- Calcula el promedio de las notas y lo redondea a 2 decimales
...
SELECT e.id_estu AS ID, e.nombre, e.apellido, e.matricula, 'Estudiante' AS tipo FROM estudiantes e WHERE e.id_usu = 9
UNION 
SELECT p.id_profe AS ID, p.nombre, p.apellido, p.matricula, 'Profesor' AS tipo FROM profesores p WHERE p.id_usu = 9;
